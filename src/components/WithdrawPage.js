...
function WithdrawPage(props) {
    ...
    function withdrawFunds(){
        if(parseInt(amount)!==0 && amount!==""){
            if(parseInt(amount) <= parseInt(users[usersIndex].balance)){
                ...
                userTransactions.push({"transactionType": "Withdrawn Funds", "transactionAmount": `-₱ ${parseInt(amount)}`, "timeOfTransaction": date})
    ...
    return (
        <div className="w-full py-5 text-center fade-in">
                ...
                <button className="my-3 bg-pink-500 hover:bg-pink-600 text-white font-bold py-2 px-4 border border-pink-700 rounded w-50 m-auto disabled:opacity-50" onClick={withdrawFunds} disabled={parseInt(amount)===0 || amount==="" || !(parseInt(amount) <= parseInt(users[usersIndex].balance))}>
                    Withdraw Funds
                </button><br></br>
